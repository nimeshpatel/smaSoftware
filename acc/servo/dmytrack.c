 ...
 * $Log: dmytrack.c,v $
 * Revision 1.14  2013/02/08 14:11:41  rwilson
 * put 50 ms delay in main loop
 *
 ...
 *
 */
static char rcsid[] = "$Id: dmytrack.c,v 1.14 2013/02/08 14:11:41 rwilson Exp $";
...
/* Communication from the User Interface to driver, all in degrees. */
double newP = 0, newV = 0;	/* New az or el position and velocity */
double newA = 0;		/* New Acceleration */
int newAz = 0, newEl = 0;	/* Set newAz to 1 if new[PV] are new az */
/* Keep the acceleration in mas/sec to be consistent with the tsshm
 * values for az, el, azVel and elVel */
double azAccel = 0, elAccel = 0;
...
int main(int argc, char *argv[]) {
	...
	for(;;) {
		...
		case 'a':
		    if(line[1] == 'a') {
			n = sscanf(ip, "%lf %lf %lf", &newP, &newV, &newA);
			...
			case 2:		/* This is supposed to fall through */
			    newA = 0;
			case 3:		/* This is supposed to fall through */
			    if(line[2] == 's') {
				newP = tsshm->az/MSEC_PER_DEG + newP/3600;
				newV = tsshm->azVel/MSEC_PER_DEG + newV/3600;
				newA = azAccel/MSEC_PER_DEG  + newA/3600;
			    } else {
				newP = tsshm->az/MSEC_PER_DEG + newP;
				newV = tsshm->azVel/MSEC_PER_DEG + newV;
				newA = azAccel/MSEC_PER_DEG  + newA;
			    }
			    /* Signal driver new az pos, vel. if accel is ok */
			    newAz = CheckNewA();
			    break;
			default:
			    printf("??\n");
		...
		case 'e':
			...
			case 3:		/* This is supposed to fall through */
				...
				newA = elAccel/MSEC_PER_DEG  + newA/3600;
			    } else {
				newP = tsshm->el/MSEC_PER_DEG + newP;
				newV = tsshm->elVel/MSEC_PER_DEG + newV;
				newA = elAccel/MSEC_PER_DEG  + newA;
			    }
			    /* Signal driver new el pos, vel. if accel is ok */
			    newEl = CheckNewA();
			    break;
			default:
			    printf("??\n");
	    ...
	    free(line);
	    /* Allow servo to digest this change before any more are given */
	    usleep(50000);
	}
...
void *driver(void *dmyarg) {
	...
	static double azVel, elVel;	/* Vel cmds to servo (mas/sec) */
	...
	for(;;) {
		...
		if(newAz) {
		    ...
		    tsshm->azVel = azVel = newV * MSEC_PER_DEG;
		    /*correct the reference elevation position to new ref time*/
		    tsshm->el = el += (tsshm->elVel + elAccel * dt/2) * dt;
		    azAccel = newA * MSEC_PER_DEG ;
		    msecCmd = lastMsec;
		    newAz = 0;
		    dt = 0;
		} else if(newEl) {
		    tsshm->el = el = newP * MSEC_PER_DEG;
		    tsshm->elVel = elVel = newV * MSEC_PER_DEG;
		    /*correct the reference azimuth position to new ref time*/
		    tsshm->az = az += (tsshm->azVel + azAccel * dt/2.0) * dt;
		    elAccel = newA * MSEC_PER_DEG ;
		    msecCmd = lastMsec;
		    newEl = 0;
		    dt = 0;
		}
		{
		    tsshm->az = az + (azVel + azAccel*dt/2.0)*dt;
		    tsshm->azVel = azVel + azAccel * dt;
		    tsshm->el = el + (elVel + elAccel * dt/2.0)*dt;
		    tsshm->elVel = elVel + elAccel * dt;
		}
...
int CheckNewA(void) {
	if(newA > 0.01 ||newA < -0.01) {
	    fprintf(stderr,
	        "Acceleration is too large (%.4f), ignoring request\n", newA);
	    return(0);
	}
	return(1);
}
