 ...
 * $Log: svdata.c,v $
 * Revision 1.11  2014/06/19 19:53:01  rwilson
 * retreive 2 az tachs and 3 motor currents
 *
 ...
 */
static char rcsid[] = "$Id: svdata.c,v 1.11 2014/06/19 19:53:01 rwilson Exp $";
...
7       Az Velocity from tachometer 1
8       Az Velocity from tachometer 2
9       Az Torque in Amperes (average of the two motors)
10	Az motor 1 Current
11	Az motor 2 Current
12      State of the Az command shaper
13      El command from track
14      El command out of the command shaper
15      El from the fine encoder12
16      El Velocity commanded by servo
17      El Velocity from the tach
18      El Torque (Amps)
19	El Motor current(Amps)
20      State of the El command shaper
*/
...
#define TO(x) sp->x*(70./32768.0)	/* Torque in Amps */
	for(sp = samples; sp < &samples[nextSample]; sp++) {
		...
		TO(azTorq), sp->mot1CurrentAz, sp->mot2CurrentAz, sp->azState, 
		PO(curEl), PO(shpEl), PO(encEl),
		PO(cmdElVel), PO(tachElVel), TO(elTorq),
		sp->motCurrentEl, sp->elState
	    );
	}
	return(0);
}
