 ...
 * $Log: servo.c,v $
 * Revision 1.149  2014/06/19 19:36:51  rwilson
 * Store both az tachs and all 3 motor currents for svdata
 *
 * Revision 1.148  2011/03/04 19:22:28  rwilson
 * suppress the Ait Pressure message for 15 sec after the az drive is turned on
 *
 * Revision 1.147  2009/04/20 12:28:51  rwilson
 * fix a bug in OpMessages
 *
 * Revision 1.146  2009/04/16 21:29:06  rwilson
 * Made OpMessages secure in the face of timeout delays causing half of the cycles to be skipped
 *
 * Revision 1.145  2009/04/15 23:57:41  rwilson
 * OpMsgs for low gains and Bad SCB communications
 *
 * Revision 1.144  2009/04/11 15:15:44  rwilson
 * New Collision limit code and CheckResets
 *
 * Revision 1.143  2008/08/14 13:46:26  rwilson
 * Don't fail with one packet error
 *
 * Revision 1.142  2008/05/14 12:22:54  rwilson
 * save all parts of sample
 *
 * Revision 1.141  2008/05/09 18:44:06  rwilson
 * Add dump of previous state on unexpected drive shutdown
 *
 * Revision 1.140  2007/02/06 00:00:24  rwilson
 * collision pad changes
 *
 * Revision 1.139  2006/12/07 21:13:17  rwilson
 * reverse sense of scbStowRequest
 */
static char rcsid[] = "$Id: servo.c,v 1.149 2014/06/19 19:36:51 rwilson Exp $";
...
enum ENCTYPE azEncType, elEncType;
...
int main(int argc, char *argv[]) {
  ...
  if(sscanf(getenv("HOST"), "acc%d", &myAntennaNumber) != 1 ||
      myAntennaNumber < 0 || myAntennaNumber > 10) {
    myAntennaNumber = 0;
  }
  antmsg[8] = myAntennaNumber + '0';
...
read(scfd, &sctime2, sizeof(sctime2));
      ...
      /* Loop performance data */
        ...
        SaI.elTorq = ComGetS();
        SaI.motCurrentEl = (ADCFACTOR*20)*(0x03ff&ComGetS());
        SaI.tach1AzVel = (((int)ComGetS()) << 1) / azVel2Scb;
        SaI.tach2AzVel = (((int)ComGetS()) << 1) / azVel2Scb;
	tsshm->tachAzVel = (SaI.tach1AzVel+SaI.tach2AzVel)/2;


        /* SaI.tachAzVel = tsshm->tachAzVel =  (((int)ComGetS()) << 1) /
                                            azVel2Scb; */
        SaI.azTorq = ComGetS();
        SaI.mot1CurrentAz = (ADCFACTOR*20)*(0x03ff&ComGetS());
        SaI.mot2CurrentAz = (ADCFACTOR*20)*(0x03ff&ComGetS());
#endif /* SIMULATING */
...
void PrintSamples(void) {
  ...
  while(!SEMPTY) {
    ...
    fprintf(fp, "%7.3f %2d"
	/* Az values */
	"% 11.5f %11.5f %11.5f %8.4f %8.5f %8.5f %5.3f %5.1f %5.1f %1d"
	/* El values */
	"% 11.5f %11.5f %11.5f %8.4f %8.5f %5.3f %5.1f %1d"
	"\n", SaO.msec*0.001, SaO.scbStatus,
	PO(curAz), PO(shpAz), PO(encAz),
	PO(cmdAzVel), PO(tach1AzVel), PO(tach2AzVel),
	TO(azTorq), SaO.mot1CurrentAz, SaO.mot2CurrentAz, SaO.azState, 
	PO(curEl), PO(shpEl), PO(encEl),
	PO(cmdElVel), PO(tachElVel), TO(elTorq),
	SaO.motCurrentEl, SaO.elState
#if 0
