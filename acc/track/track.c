...
new_source:                    
	...
	/* write them to RM */
        rm_status=rm_write(RM_ANT_0,"RM_POLAR_MJD_L",&polar_mjd);
...
*/
   ...
   rm_status=rm_write(RM_ANT_0,"RM_HOTLOAD_HIGHFREQ_VOLTS_F",&dummyFloat);
	...
	if(syncdet2StaleFlag==0) {
	  /*
	  printf("syncdet2StaleFlag = %d\n",syncdet2StaleFlag);
	  */
	  rm_status=rm_read(RM_ANT_0,"RM_CONT_DET_MUWATT_V2_F",&contDetMuWatt);
	  sky_volts = (double) contDetMuWatt[0];
	  sky_volts2 = (double) contDetMuWatt[1];
	  /*
	  printf("syncdet2StaleFlag = %d, sky_volts=%f \n",syncdet2StaleFlag,sky_volts);
	  */
	}
	if(syncdet2StaleFlag==1) {
	  rm_status=rm_read(RM_ANT_0,"RM_CONT1_DET1_POWER_MUWATT_F",&dummyFloat);
...
*/
	...
	rm_write(RM_ANT_0,"RM_SUN_DISTANCE_DEG_D",&sundistance);
...
/****************************end of motion control*********************/
	...
	rm_status=rm_read(RM_ANT_0,
	     "RM_AFT_FOREWARD_TILT_UPPER_CELESTIAL_D",&scaledTiltx);
	rm_status=rm_read(RM_ANT_0,
	     "RM_LEFT_RIGHT_TILT_UPPER_CELESTIAL_D",&scaledTilty);
...
/* if tiltflag was set for online corrections, enable it if in tracking
mode */
	/* check if logTilts is running */
	rm_read(RM_ANT_0,"RM_TILT_TIMESTAMP_L",&logTiltsTS);
        rm_read(RM_ANT_0,"RM_UNIX_TIME_L",&timestamp);
          if (logTiltsTS < (timestamp-60)) logTiltsRunning=0;
	   	else { 
		logTiltsRunning=1;
		logTiltsErrorMsgSent=0;
		}

	if((logTiltsRunning==0) && (!logTiltsErrorMsgSent)) {
	sprintf(operatorErrorMessage,
	"logTilts is not running. Real-time tilt correction disabled.\n");
	sendOpMessage(OPMSG_WARNING,11,60,operatorErrorMessage);
	logTiltsErrorMsgSent=1;
	}

	if(setTiltflag==1) {

	/* check if RM_TILT_FLAGS is OK */
	rm_read(RM_ANT_0,"RM_TILT_FLAGS_S",&rmTiltFlagBits);

/*
	if (logTiltsRunning && (!(0x0 & rmTiltFlagBits)) && 
			      (!(0x1 & rmTiltFlagBits)) &&
				((tsshm->azState)==3)) tiltflag=1;
*/
	if (logTiltsRunning && (!(0x0 & rmTiltFlagBits)) && 
			      (!(0x1 & rmTiltFlagBits)) 
		&& (fabs(scaledTiltx) < 1800.) && (fabs(scaledTilty) < 1800.)) 
		 	tiltflag=1;
		else {
		tiltflag=0;
	   	strcpy(operatorErrorMessage, 
		"Wacko tiltmeter readings- disabling tilt correction. 
		Check logTilts.");
           	sendOpMessage(OPMSG_WARNING, 10, 30, operatorErrorMessage);
		}
...
/* Adding a sanity check for scaled tiltmeter readings as written
by logTilts- in case logTilts is running and still has failed to write 
appropriate values */

	 rm_status=rm_write(RM_ANT_0,"RM_TILT_CORRECTION_FLAG_L",
                                        &tiltflag);
/*
printf("setTiltFlag=%d logTiltsRunning=%d rmTiltFlagBits=%d tiltflag=%d\n",
	setTiltflag,logTiltsRunning,rmTiltFlagBits,tiltflag);
*/
...
*/
	...
	case '1':
        printf("Got command 1 for ra/dec offset.");
	strcpy(lastCommand,"Adding ra/dec offset");
	SendLastCommandToRM(lastCommand);
	rm_status=rm_read(RM_ANT_0,"RM_RAOFF_ARCSEC_D",&raOffset);
	rm_status=rm_read(RM_ANT_0,"RM_DECOFF_ARCSEC_D",&decOffset);
        printf("raOffset=%f decOffset=%f\n",raOffset,decOffset);
		icount=0;
...
*/
	...
	rm_status=rm_write(RM_ANT_0,"RM_PMAZTILT_AZOFF_D",&pmaztiltAzoff);
	rm_status=rm_write(RM_ANT_0,"RM_PMAZTILT_ELOFF_D",&pmaztiltEloff);
...
is_planet(char *s, int *flag, int *id)
{
	...
	*flag = 1;
    } else if (!strcmp(s, "2005yu55            "))
    {
  	*id = 386;
	*flag = 1;
    } else if (!strcmp(s, "c2012s1             "))
    {
  	*id = 387;
	*flag = 1;
    } else if (!strcmp(s, "c2012k5             "))
    {
  	*id = 388;
	*flag = 1;
    } else if (!strcmp(s, "c2011l4             "))
    {
  	*id = 389;
	*flag = 1;
    } else if (!strcmp(s, "c2012f6             "))
    {
  	*id = 390;
	*flag = 1;
    } else if (!strcmp(s, "1998qe2             "))
    {
  	*id = 391;
	*flag = 1;
    } else
...
{
/*
  return((tjd-2451545.)* 86400.0+32.184+32.+polar_dut);
*/
