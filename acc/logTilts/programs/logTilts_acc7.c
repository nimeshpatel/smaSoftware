...
{
  int i, fD[N_TILTS+6], nRead, status;
  short data;
  unsigned loopCount = 0;
  int antList[RM_ARRAY_SIZE];
  int gotSecondAD, secondADFD;
  float fData[N_TILTS+6], fSecond, boxCar, vEncoderSupply[4];
  char nodeName[80];
  int debugMessagesOn;
  double voltage;
...
/*********************************************************************/   
  ...
  while (1) {
	...
	exit(-1);
      ...
      if (status != RM_SUCCESS) { 
	rm_error_message(status, "rm_write(TOTAL_POWER_VOLTS_D)");
	exit(-1);
      } 
      voltage = (double)fData[5];
      status = rm_write(RM_ANT_0, "RM_TOTAL_POWER_VOLTS2_D", &voltage);
      if (status != RM_SUCCESS) { 
	rm_error_message(status, "rm_write(TOTAL_POWER_VOLTS2_D)");
	exit(-1);
      } 
...
#if 0
      /* this RM var no longer exists */
      voltage = (double)(boxCar/99.0);
      status = rm_write(RM_ANT_0, "RM_IF_LO_TOTAL_POWER_D", &voltage);
      if (status != RM_SUCCESS) { 
	rm_error_message(status, "rm_write(IF_LO_TOTAL_POWER_D)");
	exit(-1);
      } 
#endif
      for (i = 0; i < 4; i++) {
	vEncoderSupply[i] = fData[6+i];
      }
      status = rm_write(RM_ANT_0, "RM_ENCODER_SUPPLY_VOLTAGE_V4_F", vEncoderSupply);
      if (status != RM_SUCCESS) {
	rm_error_message(status, "rm_write(RM_ENCODER_SUPPLY_VOLTAGE_V4_F)");
	exit(-1);
      }
    }
    usleep(10000);
