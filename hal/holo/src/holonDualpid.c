...
{
    FILE       *fp_raw_data, *fp_raw_data2, *fp_pid
    union 	ad_union amp_vvm[10], pha_vvm[10];
    union 	ad_union amp_vvm2[10], pha_vvm2[10];
    char	command[35],file_name[80],file_name2[80],*cfreq,c,padID,padID2,padIDR;
...
/* Dual */
	...
	rm_status=rm_read(ant_scn,"RM_PAD_ID_B",&PadID1);
	rm_status=rm_read(ant_scn2,"RM_PAD_ID_B",&PadID2);
	rm_status=rm_read(ref_ant,"RM_PAD_ID_B",&PadIDR);

	fp_pid = fopen(/application/holo/includeFiles/padBoreFocus.conf,"r");

	for (i=0;i<PadID1;i++){
	fscanf(fp_pid,"%f %f",az_bore, el_bore, chopperZ);
	}
	for (i=0;i<PadID2;i++){
	fscanf(fp_pid,"%f %f",az_bore2, el_bore2, chopperZ2);
	}
	for (i=0;i<PadIDR;i++){
	fscanf(fp_pid,"%f %f",az_boreR, el_boreR, chopperZR);
	}
...
/* moving rm_open to front for pad_id */
/*	rm_status=rm_open(antlist);
	if(rm_status != RM_SUCCESS) {
		rm_error_message(rm_status,"rm_open()");
		exit(1);
...
/* get pad IDs */
rm_status=rm_read(ant_scn,"RM_PAD_ID_B",&PadID); 
printf("Antenna %d is on pad %d \n", ant_scn,PadID); 
rm_status=rm_read(ant_scn2,"RM_PAD_ID_B",&PadID); 
printf("Antenna %d is on pad %d \n", ant_scn2,PadID2); 
rm_status=rm_read(ref_ant,"RM_PAD_ID_B",&PadID); 
printf("Antenna %d is on pad %d \n", ref_ant,PadIDR); 
