...
/* 
  ...
  commanding only one antenna to scan. 
  It is identical otherwise to holon and holo2 - T.K. Nov 19, 2013
*/
/************************************************************************
 *
 *  File Name: holon.c 
...
{
    FILE       *fp_raw_data, *fp_raw_data2, *fp_file1, *fp_file2, *fp_done;
    union 	ad_union amp_vvm[10], pha_vvm[10];
    union 	ad_union amp_vvm2[10], pha_vvm2[10];
    char	command[35],file_name[80],file_name2[80],file_nameTmp[80],*cfreq,c;
...
/* Dual */
/* include _2 in the 2nd antenna's data file name */
	sprintf(file_name2, "/data/engineering/holo/ant%1d/%02d%02d_%02d%02d.holo%1d_2",ant_scn2,ts.tm_mon+1,ts.tm_mday,ts.tm_hour,ts.tm_min,ant_scn2);
...
/* End Dual */
        fp_file1 = fopen("/data/engineering/holo/fileName1","w");
	sprintf(file_nameTmp, "/sma/rtdata/engineering/holo/ant%1d/%02d%02d_%02d%02d.holo%1d",ant_scn,ts.tm_mon+1,ts.tm_mday,ts.tm_hour,ts.tm_min,ant_scn);
        printf("%s\n", file_nameTmp);
        fprintf(fp_file1, "%s\n", file_nameTmp);
	fclose(fp_file1);
	sprintf(file_nameTmp, "/sma/rtdata/engineering/holo/ant%1d/%02d%02d_%02d%02d.holo%1d_2",ant_scn2,ts.tm_mon+1,ts.tm_mday,ts.tm_hour,ts.tm_min,ant_scn2);
        fp_file2 = fopen("/data/engineering/holo/fileName2","w");
        printf("%s\n", file_name2);
        fprintf(fp_file2, "%s\n", file_nameTmp);
	fclose(fp_file1);
        fp_done = fopen("/data/engineering/holo/HoloDone","w");
        printf("HoloDone: 2\n");
        fprintf(fp_done, "2\n");
	fflush(fp_done);
	fclose(fp_done);
...
/* Dual */
/* Comment out RM interrupt for second antenna */
...
/* Dual */
/* commnet out 2nd antenna moves below */
/*	printf ("Going to bore-sight position \n");
...
/*		sprintf(command, "azoff -a %d -s %f", ant_scn, azoff);
		...
		system (command);
		usleep(10);*/
		system("touch /data/engineering/holo/HoloDone");
    	} 
...
/* Dual */
/* comment out */
/*	sprintf(command, "azoff -a %d,%d -s 0", ant_scn2); 
	system (command);
	sleep(10);
	printf ("command: %s ", command);
*/
