...
float startAzOff, startElOff,azoff,eloff,az_step,el_step;
...
/*    {"stuff", 's', SMAPOPT_ARG_NONE, &stuff, 0, "Apply derived offsets at end of all reps."}, */
    ...
    */
    ...
    {"start", 'n', SMAPOPT_ARG_INT, &n_start, 0, "Starting raster grid position"}, 
    SMAPOPT_AUTOHELP
    { NULL, '\0', 0, NULL, 0 },
...
/* Raster starts
   ...
   startAz
   startEl */
        az_step=7734.0/fRest;el_step=az_step; /*23.3 for 332 GHz, and 33.3 for 232.4 GHz*/
        ...
        startAzOff=((float)n_size/2.+1.)*az_step;
        startElOff=((float)n_size/2.)*el_step - n_start*el_step;
	azoff=startAzOff;
	eloff=startElOff;
        newElOff(ant, eloff);
/* shold wait for antenna to reach? */
/* Elevation loop */
        for (j=n_start; j<n_size; j++){  /*step through rows*/
        	newAzOff(ant, startAzOff);
			...
			newAzOff(ant, azoff);
  			for (sb = 0; sb < 2; sb++)
    			  for (a1 = 1; a1 < N_ANTENNAS; a1++)
      			    for (a2 = 1; a2 < N_ANTENNAS; a2++)
        		      if ((a1 < a2) && antennaList[a1] && antennaList[a2]) {
            			printf("Final amp[%d][%d][%d][%d] = %f\tphase[%d][%d][%d][%d] = %f %f %f\n",
                   		point, a1, a2, sb, amp[point][a1][a2][sb]*10000.0,
                   		point, a1, a2, sb, phase[point][a1][a2][sb],
				azoff, eloff);
                        fprintf(fp_raw_data,"amp[%d][%d][%d][%d] = %f\tphase[%d][%d][%d][%d] = %f %f %f\n",
                        point, a1, a2, sb, amp[point][a1][a2][sb]*10000.0,
                        point, a1, a2, sb, phase[point][a1][a2][sb],
                        azoff, eloff);
                        fflush(fp_raw_data);
