 ...
 *
  lynxos
  gcc -o tot tot.c -I$COMMONINC $COMMONLIB/encoderClient.o $COMMONLIB/rm.o
 *
 *
...
int main(int argc, char **argv)
{
   ...
   /* union       ad_union amp_vvm[10], pha_vvm[10]; */
    ...
    double	chop_x,chop_y;
    float	chop_x_av,chop_y_av,output_av,amp_av;
	...
	if(argc<6)
	{
	printf("Usage: tot <filename> <antNo> <#data> <#average(10-30)> <n_delay> \n"); 
	exit(0);
	}
...
/* open Device tables */
	...
	for(l=0;l<num_of_dat;l++){
    	chop_x_av=0.;chop_y_av=0.;output_av=0.;
	amp_sum=0;amp_av=0.;az_av=0.;el_av=0.;
...
/* read encoders using encoderClient */
		...
		rm_status=rm_read(ant_scn,"RM_AZOFF_D",&chop_x);
		if(rm_status != RM_SUCCESS) {
		rm_error_message(rm_status,"rm_read()");
		exit(1);
		}
		rm_status=rm_read(ant_scn,"RM_ELOFF_D",&chop_y);
		if(rm_status != RM_SUCCESS) {
		rm_error_message(rm_status,"rm_read()");
		exit(1);
		}
...
/*		printf("%8.4f %7.4f %4d %6.2f %6.2f %6.2f %5d %5d\n",az_enc,el_enc,l,chop_x_av,chop_y_av,chop_z_av,amp,pha);*/
		...
		printf("%8.4f %7.4f %4d %6.2f %6.2f %6.2f\n",az_av,el_av,l,chop_x_av,chop_y_av,amp_av);
		fprintf(fp,"%8.4f %7.4f %4d %6.2f %6.2f %6.2f\n",az_av,el_av,l,chop_x_av,chop_y_av,amp_av);
	}
