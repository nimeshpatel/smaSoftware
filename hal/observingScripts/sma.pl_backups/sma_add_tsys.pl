...
#Begin executing target loop:
	    ...
	    {
		$i = $partialobsloop;
		print "Starting with the $i source in the loop.\n";
		$partialloopcheck = 0;
	    }
		...
		else
		{
			...
			command("observe -s ${$T_access} -n $T_name");
...
#Begin executing target loop:
		...
		else
		{
			...
			else
			{
			    ...
			    command("observe -s ${$obs_loop[$i]} -n ${$names[$i]}");
			}
			command("tsys");
			command("integrate -s ${$int_time} -t $inttime -w");
			$lastsource = ${$obs_loop[$i]};
...
#		}
	        if($targel < $MAXEL_GAIN and $targel > $MINEL_GAIN)
		{
			...
			$tempname =~ s/pol+//;
			command("rotateWaveplate -a $pants -s R");
			command("observe -s ${$tempname} -n ${$obs_loop[$i]}");
			$finished = 1;
		    }
		    else
		    {
