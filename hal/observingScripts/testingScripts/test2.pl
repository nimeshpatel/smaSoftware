#!/usr/bin/perl -w
#
# Experiment Code: 2006-09-S016
# Experiment Title: The Massive Protostellar Disk-Outflow System of G192.16-3.84
# PI: Michael A. Reid
# Contact Person: Michael A. Reid  
...
####################  PRIMING  ####################  
#
# restartCorrelator -d -s256 -s01:0 -s02:512 -s03:512 -s04:0 -s21:128 -s22:128 -s23:512 -s24:128  
#
# observe -s G192 -r 05:58:13.53 -d +16:31:58.3 -e 2000 -v 5.7
#
# dopplerTrack -r 265.895 -l -s23 
#
# setFeedOffset -f 345
#
###################   POINTING  ###################  
...
#
$inttime=30; 
$targ0="G192 -r 05:58:13.53 -d +16:31:58.3 -e 2000 -v 5.7";
$ntarg0=30; 
$cal0="0528+134"; 
$ncal0=8; 
$cal1="0507+179"; 
...
$myPID=$$;
command("project -r -p 'Michael A. Reid' -d '2006-09-S016' -i $myPID");
...
if(!$restart){
     ...
     $total_scans = 0;
     LST(); $fluxel=checkEl($flux1);
     if($fluxel > $MINEL_FLUX and $fluxel < $MAXEL_FLUX){
          print "\n";
          print "---- Observing $flux1. ----\n"; 
          print "\n";

          $current_source = $flux1;
          until($nflux1 <= $total_scans){
               LST(); $fluxel=checkEl($flux1);
               if($fluxel < $MINEL_FLUX or $fluxel > $MAXEL_FLUX){
                    print "Source elevation limit reached.\n";
                    print "Exiting ...\n";
                    ...
                    command("observe -s $flux1 -t flux");
                    if($total_scans == 0){
                         command("tsys");
                    }
                    command("integrate -s 10 -w");
                    $total_scans = $total_scans+10;
                    print "Finished $total_scans/$nflux1 on $flux1.\n";
               }
          }
      }
     $total_scans = 0;
     ...
     $total_scans = 0;
     LST(); $bpel=checkEl($bpass2);
     if($bpel > $MINEL_BPASS and $bpel < $MAXEL_BPASS){
          print "\n";
          print "----  Observing $bpass2. ----\n"; 
          print "\n";

          $current_source = $bpass2;
          until($nbpass2 <= $total_scans){
               LST(); $bpel=checkEl($bpass2);
               if($bpel < $MINEL_BPASS or $bpel > $MAXEL_BPASS){
                    print "Source elevation limit reached.\n";
                    print "Exiting ...\n";
                    ...
                    command("observe -s $bpass2 -t bandpass");
                    if($total_scans == 0){
                         command("tsys");
                    }
                    command("integrate -s 10 -w");
                    $total_scans = $total_scans+10;
                    print "Finished $total_scans/$nbpass2 on $bpass2.\n";
               }
          }
      }
}
