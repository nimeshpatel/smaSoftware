...
$FREQ = 345;    #Which receivers are you using (230,345)?
$MINFlux = 2;  #How bright do you need the pointing sources(in Jy)?
$MINDate = 20070101;   #How old can the flux measurement be (i.e. yyyymmdd) 
...
while(1) {
   ...
   if (defined($cal1)) {
        ...
        if ($cal1El < $ElUpperLimit) {
           ...
           command("ipoint -a 3..8 -i 30 -r 1 -Q -s");
        } else {
