...
$FREQ = 230;    #Which receivers are you using?
$MINFlux = 2;  #How bright do you need the baseline script sources?
$MINDate = 20070101;   #How old can the flux measurement be (give number, i.e. first of the month = 1) 

print "\n\n";
print "Fetching calibrator list...\n";

my $html_text = &get_callist_html;     
$html_text || die "Can't fetch calibrator list from http://sma1.sma.hawaii.edu/callist.html";
...
sub get_callist_html {
  ...
  print W "GET /callist.html HTTP/1.0\n\n";
  my $html;
  while($html = <R>) {
