#!/usr/bin/perl -w
#Edited to adjust elevation limits to 33deg for subcompact configuration.  
...
#Current ipoint command is 'ipoint -i 45 -r 1 -Q -s -k' for main cal and 'ipoint -i 60 -r 1 -Q -k'
#for other source.  Please change this line regarding the current ipoint command to reflect any edits you make.
#
#

# correlator integration time for each scan in seconds 
 $inttime=5;
# primary calibrator cal1 (use J2000 name);  
...
$MINDate = 20100901;   #How old can the flux measurement be (i.e. yyyymmdd) 
...
while(1) {
   ...
   for $n (1..$nsrc) {    
      ...
      command("ipoint -i 60 -r 1 -Q -k");  
   }  
      
}  ### end of the while(1) loop

# last calibration on cal1
if (defined($cal1) and checkEl($cal1) > 33) {
   command("observe -s $cal1");  command("antennaWait");
   command("ipoint -i 45 -r 1 -Q -s -k");
}  
