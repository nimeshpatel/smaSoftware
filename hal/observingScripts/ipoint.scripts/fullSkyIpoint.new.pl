...
#This script fetches quasars for the source list from 
#/global/catalogs/callist/recent_flux_measurements.dat.  Using this source  
...
#Current ipoint command is 'ipoint -i 30 -r 1'.
#
#
...
$MINDate = 20080901;   #How old can the flux measurement be (i.e. yyyymmdd) 

print "\n\n";
print "Fetching calibrator list...\n";

my @callist = get_cal_list();
for my $src (@callist) {
  if($src->[1] !~ /^\s+$/) {
    print "found source ",$src->[1],"\n";
  }
  print "  ".join ' | ', @$src;
}
...
sub get_cal_list {
  my $filename = 
    shift || '/global/catalogs/callist/recent_flux_measurements.dat';
  my @list = ();

  open FLUX, $filename or die "$filename: $!";
  while(my $nextline = <FLUX>) {
    next if $nextline =~ /^\s*#/;
    my @entries = split ',',$nextline;
    splice @entries, 1, 1; # remove B1950 name
    push @list, [ @entries ];
  }
  close FLUX;
			...
			$flux_day[$i] = $each_source[$j+1];
			 $flux_date[$i] = $flux_year[$i] . $flux_month[$i] . $flux_day[$i];
                        ...
                        {       #Assign j2000 names into the sourcelist.  
                                ($common[$i], $b1950[$i], $j2000[$i], $ra[$i], $dec[$i]) = split(' ',$sources[$i],5);
