...
while(1) {
   ...
   if (defined($cal1)) {
   	...
   	if ($cal1El < $ElUpperLimit) {
		...
		command("ipoint -i $ginteg -r 1 -Q -s -k");
			...
			$newSource = $cal1."-3";
			command("observe -s $cal1 -n $newSource");  command("antennaWait");
			command("ipoint -i 30 -r 1 -Q -3");
		}
   	} else {
   ...
   for $n (1..$nsrc) {
	...
	$newSource = $source."-3";
	command("observe -s $source -n $newSource");   command("antennaWait");
	command("ipoint -i $integ -r 1 -Q -3");  
    }
}
...
if (defined($cal1) and checkEl($cal1) > $ElLowerLimit) {
   ...
   if ($opt_kazushi) {
	command("ipoint -i $ginteg -r 1 -Q -s -k");
       ...
       $newSource = $cal1."-3";
       command("observe -s $cal1 -n $newSource"); command("antennaWait");
       command("ipoint -i $ginteg -r 1 -Q -3");
   }
}
