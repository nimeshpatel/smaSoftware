...
###########################################################################
#Modifications on Aug 31, 2009 by Ryan Howie
#I changed the way that the program fillters calibrators.  It now uses
#the flux data from the file and will only allow calibrators over 2 Jy
#at the current frequnecy to be used.
###########################################################################
...
$highpriority[4] = "0830+241";
$highpriority[5] = "0721+713";
...
$ELULIMIT=86.4; # Upper Elevation Limit of the SMA, Be careful!!!!!
...
GetOptions('restart','calibrator=s'=>\$calib,'late','early','help','subcompact','time','antenna=s'=>\$antList);
...
if($opt_help && $opt_subcompact && $opt_restart && $opt_time) #this will get rid of the only used once message.
{
...
#    $dir = "/sma/rtData/flux/mir_data/*";           #this directory isn't visable on hal
	...
	open(LOG, "<$filename[0]") or die "open failed";
	$line = $_;
	@stuff = split(" ", $line);
	$sources = $stuff[0];
	while(<LOG>)  
...
###############
sub Usage(){
printf "Usage: nflux_main.pl -c (calibrator name)
options:
...
-t (time, sets the integration time to 15 seconds and increases the number of scans)
...
#		print "The current source is: $fsource, and it's most recent $FREQ flux measurement was on: $newfluxdate\n"; 
		    #$fluxdata[8] has the flux measurement.
		    if($fluxdata[8] > 1.5)
		    {
			$cal[$calnum] = $fsource;
