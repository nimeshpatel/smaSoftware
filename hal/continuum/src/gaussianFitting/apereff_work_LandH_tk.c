...
(0) errors will occur if the measurement frequency is near an atmospheric
    absorption band: takes no cognizance of frequency dependent differences 
    between USB and LSB gains; assumes sky, hot and ambient beam terminations.
...
(4) source coupling efficiency taken into account:
    beam modelled as gaussian and source as top hat
...

{
	...
	char  resultsDat[200], summaryDat[200];
	char *ptr;
	...
	if (fpo == NULL){
#if __linux__
	sprintf(summaryDat,"/sma/rtData/engineering/rpoint/ant%d/etaSummary.%s",ant_num,rxString);
	sprintf(resultsDat,"/sma/rtData/engineering/rpoint/ant%d/etaResults.%s",ant_num,rxString);
#else
...
d m y   h m          GHz   \"    K     \"      mV     mV    mV         deg  K      K     K          nep  nep   K    K       \"               \"\n"); 
	...
	x=PlanetDia/WidthFwhm;
        /* following part from Nimesh for deconvolution */
	/* checked against FWHM actual convolution of a top hat and a beam Scott generated */
	x *= 5.;
	...
	/*
	    ...
	    /* If the source brightness temperature is the correct Planck 
	       Blackbody Kelvin the expression in the following line should be used */
	    /*
	      Jbrt=1.0/(exp(0.048*Frequency/TBright)-1);
	    */
	    /* In the following line, R-J brightness temperature definition 
	       is assumed; but not R-J approximation */
	    ...
	    /* for now it is a gaussian beam with a top hat source */
	    ...
	    /* ratio gives the source strength when scaled by Tcal=Thot=Tatm=Tamb in the simple 
	      0th order approach: so-called chopper-wheel calibration of Penzias & Burrus fame */
	    /* for us, all the corrections are incorporated into a modified Tcal or Jcal in the 
	       non-R-J case */
