...
27 Apr 06 TK: aperEff has a new command line argument offline (1/0) to invoke
              the aperOffLine.Inp file for offline parameters. This file now
              has just one line of parameters
*/
...

{
	...
	char *ptr;
        ...
        if ( argc < 3) {
                printf("usage: aperEff filename offline(1/0)  \n");
                exit(1);
        }
        useOff=atoi(argv[2]);
        if(useOff==1) printf("OFFLINE PARAMETERS FOR APERTURE EFF.!\n");
        fpi = fopen(argv[1], "r");
	if (fpi == NULL) {
...
/* Indeed it is here! we now interpolate using data from simulated scans as below*/
#if __linux__
