...
/* 	following for aperture efficiency 16 Nov 04, TK */
	...
	float tau_zenithL, TatmL, eta_lL,FrequencyL, tau_zenithH, TatmH, eta_lH, FrequencyH;
...
/*    PGPLOT */
	...
	if (fpsummary == NULL) {
	  ...
	  */
	  ...
	  fscanf(fph_eta, "%s %f %d %f %d %f %d %f %d %f %d %f %d %f %d %f %f", rawfilename, &VhotL, &time_stamp, &VhotH, &time_stamp, &VskyL, &time_stamp, &VskyH, &time_stamp, &tau_zenithL, &TatmL , &eta_lL, &FrequencyL, &tau_zenithH, &TatmH , &eta_lH, &FrequencyH, &time_stamp, &SB);
! side band above SB, is it in the header? CHECK 04 Feb 05
	  if (lowfreqflag == 1) {
		Vhot=VhotL;
		Vsky=VskyL;
