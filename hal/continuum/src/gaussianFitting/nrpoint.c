...
#include "nr.h"
#include "rpoint.h"
/* changes made to allow dual frequecny function 18 Oct 05 */
/* Planet brightness temperatures for aperture efficicecny 17 Nov 04, TK*/
/* use the variable aperoffline passed from rpoint to invoke aperOffLine.Inp, */
/* by passing aperoffline to aperEff 27 Apr 05 TK */
#define TB_JUP 165
#define TB_SAT 140
#define TB_MARS 210
#define TB_VENUS 260
...
void nrpoint(float x[],float y[],float azy[],float ely[],float azmod[],float elmod[],float sig[],int ndata,int num_gauss,int flag,int ant_num,int plotflag,char *header, int aperoffline)
{
...
/* 	following for aperture efficiency 16 Nov 04, TK */
        ...
        float tau_zenithL,TatmL,eta_lL,tau_zenithH,TatmH,eta_lH;
        float  Frequency, FreqL, FreqH, SBL, SBH,TBright, VhotL, VhotH, VskyL, VskyH, dummy;
        float PlanetDia, WidthFwhm,fwhm_beam, EtaA, EtaB;
        char  object[20], date[30];
/* 	aperture efficiecny additions end */
	printf("aperoffline: %d\n", aperoffline);
...
*/
	  sprintf(etaCommand, "awk -F, \' (NR>=2) {print $4,$14,$29,$37,$40,$94,$96,$118}\' /sma/rtData/engineering/rpoint/ant%d/header.dat.%s > %s",ant_num,rxString,pickedTmp);
#else
/*
...
*/
	  sprintf(etaCommand, "nawk -F, \' (NR>=2) {print $4,$14,$29,$37,$40,$94,$96,$118}\' /data/engineering/rpoint/ant%d/header.dat.%s > %s",ant_num,rxString,pickedTmp);
#endif
	  printf("%s\n", etaCommand); 
	  system(etaCommand);
	  ...
	  fscanf(fph_eta,"%s %f %f %f %f %f %d %s %f %d %f %d %f %d %f %d %f %d %f %f %f %f %f %f %f %f ",object,&PlanetDia,&Tamb,&Tcab,&el,&Frequency,&SB, rawfilename, &VhotL, &time_stamp, &VhotH, &time_stamp, &VskyL, &time_stamp, &VskyH, &time_stamp, &tau_zenithL, &time_stamp, &TatmL, &eta_lL, &FreqL, &SBL,&tau_zenithH, &TatmH, &eta_lH, &FreqH);
	  printf("%s %f %d\n", object, Frequency, SB);
...
/*	  Tamb = Tcab; */
	  ...
	  */
	  Thot=Tcab;
/* Frequecny used is from the tau parameters set; this is already in GHz */
...
*/
	  ...
	  if (strstr(object,"mars")!=NULL)  TBright=TB_MARS;
          if (strstr(object,"venus")!=NULL)  TBright=TB_VENUS;
	  err=0.0;
	  printf("%s %f %d\n", object, Frequency, SB);
	  ...
	  fclose(fpi_eta);
	  sprintf(etaCommand, "aperEff %s %d", aperInputTmp, aperoffline);
	  printf("Calling: %s\n",etaCommand);
