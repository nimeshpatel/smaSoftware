...
fabs(chisq) < .1) || (k>10))
			...
			fprintf(fp4,"\n\n");
	fclose(fp2);
	sprintf(f_line,"cp load.initial.dat load.initial.dat.bak");
	system(f_line);
...
/*    PGPLOT */
	...
	sprintf(f_line,"offset = %10.4f\n",(a[2]+a[7])/2.0);
	cpgmtxt("t",-2.5,0.55,0,f_line);
	cpgend();
		  printf("recorded! \n");
		if(flag==1) fprintf(fp5,"azfit %s | %10.5f %10.5f %10.5f %10.4f %10.4f %10.2f +- %10.2f %8.1f %8.1f\n",header,(azy[i_maxy]+azy[i_miny])/2.,(ely[i_maxy]+ely[i_miny])/2.,(a[6]-a[1])/2.,a[3]*2.0*0.83255,a[8]*2.0*0.83255,(a[2]+a[7])/2.,rms_fac*sqrt(covar[2][2]+covar[7][7]),azmod[i_maxy],elmod[i_maxy]);
