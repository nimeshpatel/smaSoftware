...
#define EXTENDED_TIME_LIMIT 3600 /* i.e. for beamPattern */
	/*
	int totalTimeLimit = EXTENDED_TIME_LIMIT;
	*/
	...
	copyFileMutex = (pthread_mutex_t *)malloc(sizeof(pthread_mutex_t));
	pthread_mutex_init(copyFileMutex, NULL);

	if(gotport==1) {
	  iport=0;
	  if(strstr(port,delimiter)==NULL) {
	      printf("Invalid channel numbers specified. 
		Expected comma separated integers: e.g. 3,5
		or 7,8. See detailed usage by typing recordContinuum --help. Bye.\n");
	      exit(-1);
	  } else token=strtok(port,delimiter);
	...
	if (unixtime == 0) {
	  strcpy(killfile,"/tmp/killrecordContinuum");
	} else {
	  sprintf(killfile,"/tmp/killrecordContinuum.%d",unixtime);
	}
...
void *antennaThread(void *antenna) {
  FILE *fp;
	...
	float chartSyncdet2Volts[2], floatvalue;
	int copystat;
	  ...
	  while (fopen(killfile,"r") == NULL&&fileWriteFlag==1){
	    ...
	    */
		...
		if (channel[0] == 7) {
	      ...
	      /*
	      rm_status=rm_read(ant_num,"RM_CONT1_DET1_F",&floatvalue);
	      output1 = floatvalue;
	      */
