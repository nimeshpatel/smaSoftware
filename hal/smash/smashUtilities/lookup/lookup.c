...
void getNearbySources(double lst, double az, double el, double width,
		      int optical);
...
source- name required.");
    ...
    while ((c = smapoptGetNextOpt(optCon)) >= 0) {
        ...
        if(gotsource!=1) usage(-2,"No source specified","Source name is
required
 .\n");
  ...
  if((gotra==1)||(gotdec==1)) {
        ...
        "if coordinates are specified, source-name, ra, dec, epoch are
all requi
red;
...
/* if the coordinates are B1950, precess them to J2000 first*/
	...
	/* calculating julian day */
	   ...
	   tjd=tjd_upper+tjd_lower;
            ...
            ldpool_c ( "/common/catalogs/time.ker");
...
starcat(char *s, int *star_flag, struct source * observe_source)
{
	...
	fp2 = fopen("/common/catalogs/sma_catalog", "r");
    ...
    *star_flag = 0;
	...
	fprintf(stderr,"%s\n","Source catalog is corrupted");
	  */
	fprintf(stderr,"%s\n","Requested source not found in catalog");
        break;
        }
        end_of_file=0;
    }
...
void usage(int exitcode, char *error, char *addl) {
		...
		"--time UTdate_time_string (e.g.: 4 Feb 2004 12:33:44) \n"
                ...
                "--nearby or -n <boxWidth(degrees)>:  list nearby sources in the catalog within specified box\n"
                "--optical or -o:  look in the optical catalog for nearby sources\n"
		);
        exit(exitcode);
}
...
dec2=dec+width_radian;
	...
	if(optical==0) {
	fp = fopen("/common/catalogs/sma_catalog", "r");
    nsources=0;
  ...
  if(decsign==-1) {catdec=-catdec;}
	...
	if(optical==1) {
	fp = fopen("/common/catalogs/hipcat", "r");
    nsources=0;
    while ((end_of_file = fscanf(fp, 
	"%s %d %d %f %d %d %f %f %s",
