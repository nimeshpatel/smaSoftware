...
* NAP 26 August 2000 revised for antenna-4's chopper pmac 
...
void usage(int exitcode, char *error, char *addl) {
      ...
      fprintf(stderr, "Usage: chopperY [options] --cts (or -c) (absolute counts (1000counts=1mm)\n"
		"OR --mm (or -m) (absolute counts in mm)\n"
                "[options] include:\n"
                "  -h or --help    this help\n"
...
displacement (cts/mm) required.");
        ...
        optCon = smapoptGetContext("chopperY", argc, argv, optionsTable,0);
 ...
 if(gotc!=1) usage(-2,"No displacement specified","Specify counts or mm to move.\n");
	...
	for(iant=1;iant<=8;iant++) {
        ...
        sprintf(pmaccommand,"chopperCommand2 -a %d -c p5=%d",antenna,counts_int);
        system(pmaccommand);
        sleep(1);
