...
void usage(int exitcode, char *error, char *addl) {
  ...
  fprintf(stderr,"  -a<n> or --antenna <n> (n is the antenna number)\n");
  fprintf(stderr,"    (default: all antennas)\n");
  exit(exitcode);
}
...
",
  ...
  if(strcmp(unamebuf.nodename,"hal9000")) {
    ...
    usage(-1,"No antenna number specified","See value -h");
  }
		  ...
		  case 2:
		    ...
		    rm_status=rm_read(antenna,namelist[ifound],&Long);
		     ...
		     if (present(namelist[ifound],"timestamp") ||
		       ...
		       rm_status=rm_read(antenna,"RM_UNIX_TIME_L",&UnixTime);
          	       if (rm_status != RM_SUCCESS) {
                         rm_error_message(rm_status,"rm_read()");
                         exit(1);
		       }
		       if (print==1) {
			 printf("It is out of date by %d seconds\n",UnixTime-Long);
		       }
		     }
		   }
		}
