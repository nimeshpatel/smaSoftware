...
void usage(int exitcode, char *error, char *addl) {
        ...
        fprintf(stderr, "Usage: offsetUnit  [options] -s <arcseconds>\n"
                "[options] include:\n"
                "  -h or --help    this help\n"
...
/*      char messg[100]; */
	...
	offsetUnits (arcseconds) required.");

        optCon = smapoptGetContext("offsetUnit", argc, argv, optionsTable,0);
 ...
 if(gotit!=1) usage(-2,"No value given","OffsetUnit (arcseconds) is required
.\n");
	...
	rm_status=rm_write(antenna,"RM_SMARTS_OFFSET_UNIT_ARCSEC_S",
			&offset_unit);
	if(rm_status != RM_SUCCESS) {
