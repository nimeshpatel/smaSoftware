...
void usage(int exitcode, char *error, char *addl) {
        ...
        fprintf(stderr, "Usage: azoff [options] --arcseconds (or -s) <arcseconds>\n"
                "[options] include:\n"
                "  -h or --help    this help\n"
...
int main(int argc, char *argv[])  
{
	...
	char c,*arcseconds,messg[100],command_n[30];
	short pmac_command_flag=0;
	short error_flag=RUNNING;
	int gotazoff=0,rm_status,antlist[RM_ARRAY_SIZE];
	double azoff;
	int i, antenna;
...
azoff(arcseconds) required.");

        optCon = smapoptGetContext("azoff", argc, argv, optionsTable,0);
	...
	for(i=0;i<30;i++) {
        ...
        command_n[0]='O'; /* send 'O' command */

	 if(gotazoff!=1) usage(-2,"No offset specified","Az (arcseconds) is 
	required .\n");
	...
	rm_status=rm_write(antenna, "RM_SMASH_AZOFF_ARCSEC_D",&azoff);
	 if(rm_status != RM_SUCCESS) {
...
* Revised on 27 Feb 2001 for PowerPC and smapopt.
...
void  usage(int exitcode, char *error, char *addl) {
        ...
        if (error) fprintf(stderr, "\n%s: %s\n\n", error, addl);
        fprintf(stderr, "Usage: eloff [options] \n"
	  " -s <arcseconds offset> or \n"
	  " --arcseconds <arcseconds offset>.\n"
                "[options] include:\n"
                "  -h or --help    this help\n"
                "  -a<n> or --antenna <n> (n is the antenna number)\n"
...
int main(int argc, char *argv[])  
{
	...
	short  pmac_command_flag=0;
	short error_flag=RUNNING;
	int goteloff=0,rm_status,antlist[RM_ARRAY_SIZE];
	double eloff;
	int i, antenna;
	smapoptContext optCon;
	int gotantenna=0,antennaArray[SMAPOPT_MAX_ANTENNAS+1],
...
eloff(arcseconds) required.");
	...
	for(i=0;i<30;i++) {
        ...
        command_n[0]='P'; /* send 'P' command */
        pmac_command_flag=0;


 if(goteloff!=1) usage(-2,"No offset specified","Eloff (arcseconds) is required
.\n");
	...
	rm_status=rm_write(antenna, "RM_SMASH_ELOFF_ARCSEC_D",&eloff);
	 if(rm_status != RM_SUCCESS) {
                rm_error_message(rm_status,"rm_write()");
                exit(1);
        }
