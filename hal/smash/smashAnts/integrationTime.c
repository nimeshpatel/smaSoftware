...
void usage(int exitcode, char *error, char *addl) {
        ...
        fprintf(stderr, "Usage: integrationTime  [options] -s <msec>\n"
                "[options] include:\n"
                "  -h or --help    this help\n"
...
/*      char messg[100]; */
	...
	short integration_time;
	int seconds;
	int i, antenna;
	int gotit=0,rm_status,antlist[RM_ARRAY_SIZE];
	...
	int numberOfAnts=0;
	 ...
	 {"antennas",'a',SMAPOPT_ARG_ANTENNAS,antennaArray,'a'},
                {"seconds",'s',SMAPOPT_ARG_INT,&seconds,'s'},
                {NULL,0,0,NULL,0}
        };

	
	if(argc<2) usage(-1,"Insufficient number of arguments","At least
	integration-time (seconds) required.");
		...
		gotantenna=1;
                ...
                case 's':
                gotit=1;
		integration_time=(short)seconds;
                break;
                }
 ...
 if(gotit!=1) usage(-2,"No value given","Integration time (sec) is required
.\n");
	...
	{
	fprintf(stderr,"integration time cannot be negative.\n");
	exit(-1);
	}
	...
	rm_status=rm_write(antenna,"RM_SMARTS_INTEGRATION_TIME_SEC_S",
			&integration_time);
	if(rm_status != RM_SUCCESS) {
