...
void updateStripChart(int, int);
int rmvarindex = 0;
int rmvarindex2 = 0;
int pointsRead[9] = {0,0,0,0,0,0,0,0,0};
...
#define CHART_YOFFSET  80
  ...
  do {
    ...
    for (antennaNumber=1; antennaNumber <= 8; antennaNumber++) {
      rms = getNextValue(&nextvalue,antennaNumber);
      buf[antennaNumber][pointsRead[antennaNumber]] = nextvalue;
    ...
    for (antennaNumber=1; antennaNumber <= 8; antennaNumber++) {
      pointsRead[antennaNumber]++;
      rms = call_rm_read(antennaNumber,"RM_UNIX_TIME_L",&rightnow);
      rms = call_rm_read(antennaNumber,
			 "RM_HOTLOAD_LOWFREQ_VOLTS_TIMESTAMP_L",
			 &timestamp);
      if (pointsRead[antennaNumber]>=CHART_XVALUES ||((rightnow-timestamp)<2)){
	refresh();
	needToRefresh = 1;
	pointsRead[antennaNumber] = 0;
        for (i=1; i<antennaNumber; i++) {
	  updateStripChart(i,needToRefresh);
	}
      }
    }
    totalPointsRead++;
...
void updateStripChart(int antenna, int redraw) {
  int y,x;
  int i;
  int pixelsPerSecond;

  pixelsPerSecond = PIXELS_PER_SECOND;
  if (pointsRead[antenna] > 0) {
    if (redraw) {
      x = chartXoffset[antenna] + (0)*pixelsPerSecond;
      y = floor(chartYoffset[antenna]+CHART_YWIDTH*(buf[antenna][pointsRead[antenna]-1]-yminimum[antenna])/(ymax[antenna]-yminimum[antenna]));
      move(x,y);
      for (i=1; i<=pointsRead[antenna]; i++) {
	...
	line(x,y);
      ...
      x = chartXoffset[antenna] + (pointsRead[antenna]-1)*pixelsPerSecond;
      y = floor(chartYoffset[antenna]+CHART_YWIDTH*(buf[antenna][pointsRead[antenna]-1]-yminimum[antenna])/(ymax[antenna]-yminimum[antenna]));
      move(x,y);
      x  = chartXoffset[antenna] + (pointsRead[antenna])*pixelsPerSecond;
      y = floor(chartYoffset[antenna]+CHART_YWIDTH*(buf[antenna][pointsRead[antenna]]-yminimum[antenna])/(ymax[antenna]-yminimum[antenna]));
      line(x,y);
    }
  }
}
...
#define LOW_PAD 0.98
  ...
  if (autoscale) {
    ...
    if (pointsRead[antenna]>CHART_XVALUES || 1) {
      /* limits should follow entire history of the value */
      if (historicalMax[antenna] > 0) {
	if (value >= ymax[antenna] || pointsRead[antenna] < 1) {
	...
	if (value <= yminimum[antenna] || pointsRead[antenna] < 1) {
